...
namespace BaigMedicalStore.Common
{
    public class CommonFunction
    {
        public static string GetBaseUrlForActions(string actionName, string controllerName)
        {
            ...
            return  Common.AppConstants.Configuration.AppHostPrefix + partialUrl.ToString();

        }

        public static string GetSelectedNavigationLinkClass(string controller, List<string> actions, bool isParentNode = true)
        {
            string selectedNavClassName = string.Empty;
            ...
            if (routeValues.ContainsKey("controller"))
            {
                ...
                if (controller.ToLower() == currentController.ToLower() && (actions.Any(action => action.ToLower() == currentAction.ToLower()) || isParentNode))
                {
                    selectedNavClassName = "site-nav-selected";
                }
            }

            return selectedNavClassName;
        }

        public static void PopulateFiltersInHashTable(IEnumerable<IFilterDescriptor> filters, Hashtable filterTable)
        {
            ...
            if (filters.Any())
            {
                foreach (var filter in filters)
                {
                    ...
                    if (descriptor != null)
                    {
                        filterTable.Add(descriptor.Member, descriptor.Value);
                    }
                    else if (filter is CompositeFilterDescriptor)
                    {
                        PopulateFiltersInHashTable(((CompositeFilterDescriptor)filter).FilterDescriptors, filterTable);
                    }
                }
            }
        }
        ...
        public static string GetPlainTextFromHtml(string htmlString)
        {
            ...
            string stringWithoutHTML = objHtmlToText.Convert(htmlString);

            return stringWithoutHTML;
        }

        public static string TrimHtmlText(int numberOfChars, string contentText)
        {
            string plaintext = string.Empty;
            plaintext = GetPlainTextFromHtml(contentText.Substring(0, Math.Min(numberOfChars, contentText.Length)));
            if (contentText.Length > numberOfChars)
            {
                plaintext += "....";
            }
            return plaintext;
